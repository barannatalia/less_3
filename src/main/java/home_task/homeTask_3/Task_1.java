...
public class Task_1 {
    ...
    public void setAge(int age) {
        ...
        if (age > 0 && age <= 100) {
            this.age = age;
        } else System.out.println("Вік має бути від 0 до 100 років");
    }
    ...
    public void setName(String name) {
        ...
        if (nameWithoutSpaces.isEmpty()) {
            System.out.println("Ім'я не може складатися тільки з пробілів");
            return;
        }
        if (name.length() >= 3 && name.length() <= 50) {
            this.name = name.substring(0, 1).toUpperCase() + name.substring(1);
        } else System.out.println("Довжина імені має бути від 3 до 50 символів");
    }
